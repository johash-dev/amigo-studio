...
export const DASHBOARD_NAME = '_dashboard';
export const DASHBOARD_HOST = '127.0.0.1';
export const DASHBOARD_PORT = 3999;

export function dashboardPort() {
  const raw = process.env.AMIGO_DASHBOARD_PORT;
  if (raw == null || raw === '') return DASHBOARD_PORT;
  const n = Number(raw);
  if (!Number.isInteger(n) || n <= 0 || n > 65535) {
    throw new AmigoError('invalid dashboard port', { port: raw, next: 'unset AMIGO_DASHBOARD_PORT or set 1-65535' });
  }
  return n;
}
